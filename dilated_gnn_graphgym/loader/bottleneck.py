...
class BottleneckDataset(Dataset):
    def __init__(self, data_list, transform=None, pre_transform=None, pre_filter=None):
        ...
        if pre_transform is not None:
            ...
            #         data_list[i:i+n] = pool.map(pre_transform, data_list[i:i+n])
            # for i in range(len(data_list)//n, len(data_list)):
            #     data_list[i] = pre_transform(data_list[i])

         
            @ray.remote
            def f(data):
                return self.pre_transform(data)
            
            data_list = ray.get([f.remote(d) for d in data_list])
            # with torch.multiprocessing.Pool() as pool:
            #     data_list = pool.map(self.pre_transform, data_list)
...
def create_bottleneck_dataloader(name, dataset_dir, transform, pre_transform):
    ...
    dataset_train = BottleneckDataset(dataset_train, transform=transform, pre_transform=pre_transform)
