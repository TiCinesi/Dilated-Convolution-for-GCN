...
for CONFIG in "$CONFIG_DIR"bottleneck_base-name=depth_2-*.yaml; do
  if [ "$CONFIG" != "$CONFIG_DIR/*.yaml" ]; then
    sbatch --gpus=1 --wrap="python main.py --cfg $CONFIG --repeat 1 --mark_done" --time=24:00:00 --ntasks-per-node=1 --mem-per-cpu=2GB --cpus-per-task=16 -A s_stud_infk --output=logs/%j.out
  fi
done
